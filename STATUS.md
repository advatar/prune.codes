...
- Added exact repo/base/HEAD/index-digest cache validation, durable pack caches and failure ledger, shared mutation-operation schedules, complete access/cost/transition evidence, exclusive result plus digest receipt creation, an evidence-only independent verifier, seven deliberate-corruption cases, and seven protocol unit tests. Release `ce-cli` builds successfully; protocol tests and a non-SWE synthetic index/pack shape smoke pass.
